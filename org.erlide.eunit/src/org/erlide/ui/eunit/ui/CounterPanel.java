...
public class CounterPanel extends Composite {
	...
	private final Image fErrorIcon = createImage("ovr16/error_ovr.gif"); //$NON-NLS-1$
	private final Image fFailureIcon = createImage("ovr16/failed_ovr.gif"); //$NON-NLS-1$
	...
	public void setRunValue(final int value, final int ignoredCount) {
		String runString;
		if (ignoredCount == 0) {
			runString = MessageFormat.format(" {0}/{1}",
					Integer.toString(value), Integer.toString(fTotal));
		} else {
			runString = MessageFormat.format(" {0}/{1} ({2} ignored)",
					Integer.toString(value), Integer.toString(fTotal),
