...
public class FailureTrace implements IMenuListener {
	...
	public FailureTrace(final Composite parent, final Clipboard clipboard,
		...
		final OpenStrategy handler = new OpenStrategy(fTable);
	...
	public void menuAboutToShow(final IMenuManager manager) {
		if (fTable.getSelectionCount() > 0) {
			...
			manager.add(new EUnitCopyAction(FailureTrace.this, fClipboard));
		}
		// fix for bug 68058
	...
	public void updateEnablement(final TestElement test) {
		final boolean enableCompare = test != null
				&& test.isComparisonFailure();
	...
	private String[] getFilterPatterns() {
		if (EUnitPreferencesConstants.getFilterStack()) {
			return EUnitPreferencesConstants.getFilterPatterns();
		}
		return new String[0];
	}
	...
	public void setInformation(final String text) {
		clear();
		final TableItem tableItem = fFailureTableDisplay.newTableItem();
