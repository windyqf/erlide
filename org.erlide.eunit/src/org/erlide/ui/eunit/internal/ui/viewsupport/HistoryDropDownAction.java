...
/*package*/class HistoryDropDownAction extends Action {
	...
	private class HistoryAction extends Action {
		...
		public HistoryAction(final Object element, final int accelerator) {
		...
		public Menu getMenu(final Control parent) {
			...
			manager.addMenuListener(new IMenuListener() {
				public void menuAboutToShow(final IMenuManager manager2) {
					final List<?> entries = fHistory.getHistoryEntries();
					final boolean checkOthers = addEntryMenuItems(manager2,
							entries);

					manager2.add(new Separator());

					final Action others = new HistoryListAction(fHistory);
					others.setChecked(checkOthers);
					manager2.add(others);

					final Action clearAction = fHistory.getClearAction();
					if (clearAction != null) {
				...
				private boolean addEntryMenuItems(final IMenuManager manager2,
						final List<?> entries) {
					if (entries.isEmpty()) {
						return false;
					}
					...
					final int min = Math.min(entries.size(),
							RESULTS_IN_DROP_DOWN);
					for (int i = 0; i < min; i++) {
						final Object entry = entries.get(i);
						final HistoryAction action = new HistoryAction(entry,
								i + 1);
						final boolean check = entry.equals(fHistory
								.getCurrentEntry());
	...
	public HistoryDropDownAction(final ViewHistory history) {
		fHistory = history;
		fMenu = null;
