...
public abstract class TestElement implements ITestElement {
	public final static class Status {
		...
		private static Status combineProgressAndErrorStatus(
				final Status progress, final Status error) {
			if (progress.isDone()) {
				if (error.isError()) {
					return ERROR;
				}
	 ...
	 *            the test name
	 */
	public TestElement(final TestSuiteElement parent, final String id,
	...
	public void setStatus(final Status status, final String trace,
	...
	public String getModuleName() {
		return extractModuleName(getTestName());
	}

	private static String extractModuleName(final String testNameString) {
		final String[] mf = testNameString.split(":");
		return mf[0];
	}
	...
	public void setElapsedTimeInSeconds(final double time) {
		fTime = time;
	}
