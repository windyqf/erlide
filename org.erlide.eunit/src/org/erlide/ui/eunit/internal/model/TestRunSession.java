...
public class TestRunSession implements ITestRunSession {
	...
	/*
	 * TODO support remote tests public TestRunSession(final ILaunch launch,
	 * final IErlProject project, final int port) { Assert.isNotNull(launch);
	 * 
	 * fLaunch = launch; fProject = project;
	 * 
	 * final ILaunchConfiguration launchConfiguration = launch
	 * .getLaunchConfiguration(); if (launchConfiguration != null) {
	 * fTestRunName = launchConfiguration.getName(); // fTestRunnerKind =
	 * JUnitLaunchConfigurationConstants //
	 * .getTestRunnerKind(launchConfiguration); } else { fTestRunName =
	 * project.getName(); // fTestRunnerKind = ITestKind.NULL; }
	 * 
	 * fTestRoot = new TestRoot(this); fIdToTest = new HashMap();
	 * 
	 * fTestRunnerClient = new RemoteTestRunnerClient();
	 * fTestRunnerClient.startListening( new ITestRunListener2[] { new
	 * TestSessionNotifier() }, port);
	 * 
	 * final ILaunchManager launchManager = DebugPlugin.getDefault()
	 * .getLaunchManager(); launchManager.addLaunchListener(new
	 * ILaunchesListener2() { public void launchesTerminated(final ILaunch[]
	 * launches) { if (Arrays.asList(launches).contains(fLaunch)) { if
	 * (fTestRunnerClient != null) { fTestRunnerClient.stopWaiting(); }
	 * launchManager.removeLaunchListener(this); } }
	 * 
	 * public void launchesRemoved(final ILaunch[] launches) { if
	 * (Arrays.asList(launches).contains(fLaunch)) { if (fTestRunnerClient !=
	 * null) { fTestRunnerClient.stopWaiting(); }
	 * launchManager.removeLaunchListener(this); } }
	 * 
	 ...
	 * 
	 * fSessionListeners = new ListenerList(); addTestSessionListener(new
	 * TestRunListenerAdapter(this)); }
	 */
	...
	// * {@link RemoteTestRunnerClient} and translates them into high-level
	// model
	...
	// fIncompleteTestSuites = new
	// ArrayList<TestRunSession.IncompleteTestSuite>();
	//
	// fStartedCount = 0;
	// fIgnoredCount = 0;
	// fFailureCount = 0;
